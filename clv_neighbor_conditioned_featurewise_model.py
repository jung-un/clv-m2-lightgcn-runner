"""Feature-wise N/V conditioning of a shared purchase-history expression.

The model keeps the ordinary 64-dimensional user and item ID embeddings.
For each user, it obtains a one-hop aggregate of jointly learned item ID
embeddings from the same binary graph as M1.  Train-history N/V percentiles
are centred once over valid training users.  Two learned 64-dimensional
vectors then reweight the purchase-history expression feature by feature.

No item economic feature, edge weight, sample weight, external score, or
separate training stage is introduced.
"""
...
def _centred_condition(values: np.ndarray, valid: np.ndarray) -> np.ndarray:
    """Centre a condition over valid train users and keep invalid users at zero."""

    centred = np.zeros_like(values, dtype=np.float32)
    if valid.any():
        centred[valid] = values[valid] - values[valid].mean(dtype=np.float64)
    return centred


class CLVNeighborConditionedFeaturewiseLightGCN(nn.Module):
    """One LightGCN with a small feature-wise N/V user transformation."""
    ...
    def __init__(
        ...
        *,
        ...
        # Two diagonal maps: 2*d parameters instead of two d->r->d maps.
        # Zero initialisation makes the first forward exactly M1 while still
        # allowing a direct BPR gradient into both vectors.
        self.activity_axis_vector = nn.Parameter(torch.zeros(embedding_dim))
        self.value_axis_vector = nn.Parameter(torch.zeros(embedding_dim))
    ...
    def axis_corrections(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        activity = (
            shared * self.c_n[:, None] * self.activity_axis_vector[None, :]
        )
        value = shared * self.c_v[:, None] * self.value_axis_vector[None, :]
    ...
    def embeddings(self, need_value: bool = True):
        # need_value is kept for the shared evaluator interface.
    ...
    def bpr_loss(self, users, positives, negatives, gate=None, lam=0.0, weights=None):
        # gate/lam are retained only for the shared trainer interface.
    ...
    def training_gradient_diagnostics(self) -> dict:
        ...
        return {
            "activity_axis_gradient_norm": gradient_norm(
                self.activity_axis_vector
            ),
            "value_axis_gradient_norm": gradient_norm(self.value_axis_vector),
        }
    ...
    def representation_diagnostics(self) -> dict:
        ...
        def selected_std(values: torch.Tensor, valid: torch.Tensor) -> float:
            selected = values[valid.bool()]
            if not selected.numel():
                return 0.0
            return float(selected.std(unbiased=False))

        return {
            "rho": self.rho,
            "axis_parameter_count": 2 * self.embedding_dim,
            ...
            "activity_condition_mean": float(
                self.c_n[self.user_activity_valid.bool()].mean()
            ),
            "activity_condition_std": selected_std(
                self.c_n, self.user_activity_valid
            ),
            "value_condition_mean": float(
                self.c_v[self.user_value_valid.bool()].mean()
            ),
            "value_condition_std": selected_std(
                self.c_v, self.user_value_valid
            ),
            ...
            "activity_axis_vector_norm": float(self.activity_axis_vector.norm()),
            "value_axis_vector_norm": float(self.value_axis_vector.norm()),
