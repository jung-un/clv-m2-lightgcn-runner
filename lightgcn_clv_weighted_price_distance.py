"""Seed-42 screen for a minimal CLV-weighted overall-price-distance M2."""
...
CODE_VERSION = "m2-clv-weighted-overall-price-distance-historical-screen-v1"
MATCHED_MODEL_ID = "m1_matched_rho0"
MODEL_ID = "m2_clv_weighted_overall_price_distance"
ID_ONLY_MODEL_ID = "m2_jointly_trained_id_only"


@dataclass(frozen=True)
class PriceDistanceConfig:
...
def configure_price_distance_run(**overrides) -> PriceDistanceConfig:
    defaults = {
        "out_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
            "_m2_clv_weighted_overall_price_distance_historical_screen_v1"
        ),
        "baseline_result_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
    ...
    return validate_price_distance_config(
        PriceDistanceConfig(**(defaults | overrides))
    )


def validate_price_distance_config(
    cfg: PriceDistanceConfig,
) -> PriceDistanceConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
...
def preflight_summary(cfg: PriceDistanceConfig) -> dict:
    cfg = validate_price_distance_config(cfg)
    return {
        ...
        "m2": {
            "architecture": "ID(64)|CLV-weighted overall price distance(2)",
            "total_dim": cfg.id_dim + cfg.auxiliary_dim,
            "historical_clv": "q_C=training-user percentile of N_hat*V_hat",
            "user_price": (
                "purchase-amount-weighted mean of train-only overall "
                "item-price percentiles"
            ),
            "item_price": "train-only overall item-price percentile",
            "layer0_user_basis": "q_C*[2*user_price,1]",
            "layer0_item_basis": "sigmoid(a_P)*[item_price,-item_price^2]",
            "layer0_pairwise_effect": (
                "-rho*sigmoid(a_P)*q_C*((p_i-p_u)^2-(p_j-p_u)^2)"
            ),
            "price_scale_range": "(0,1); learned by the same BPR",
            "pairwise_intervention_max": cfg.rho,
            "relation_block": False,
            "q_n_minus_q_v_input": False,
            "within_category_price_input": False,
            "item_id_projection_in_auxiliary": False,
            "rho": cfg.rho,
        ...
        "reading_rule": {
            ...
            "mechanism": "high-CLV Top-10 changes and rho=0 exact non-intervention",
...
def _config_hash(cfg: PriceDistanceConfig, input_hash: str, revision: str) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
...
def _prepare(cfg: PriceDistanceConfig) -> dict:
    ...
    _, _, q_c, clv_valid = helpers.build_clv_inputs(axes)
    price = price_inputs.build_overall_price_fit_inputs(
...
def _build_model(
    prepared: dict, cfg: PriceDistanceConfig, rho: float
) -> tuple[CLVWeightedPriceDistanceLightGCN, list[torch.nn.Parameter]]:
    data = prepared["data"]
    v3.set_seed(cfg.seed)
    model = CLVWeightedPriceDistanceLightGCN(
...
def _run_arm(
    ...
    *,
    model_id: str,
    rho: float,
) -> tuple[dict, CLVWeightedPriceDistanceLightGCN]:
...
def _score_diagnostics(
    ...
    id_values, clv_values, layer0_values = [], [], []
    for start in range(0, len(pair_users), 65536):
        ...
        layer0_values.append(
            model.layer0_auxiliary_scores(user_tensor, item_tensor).cpu().numpy()
        )
    sid = np.concatenate(id_values).astype(np.float64)
    sclv = np.concatenate(clv_values).astype(np.float64)
    layer0 = np.concatenate(layer0_values).astype(np.float64)
        ...
        [
            {
                ...
                "layer0_price_basis_score_std": float(layer0.std()),
                "clv_score_mean_abs": float(np.abs(sclv).mean()),
                "per_user_clv_abs_q_c_spearman": float(level_corr),
                "price_scale": float(model.price_scale()),
                "max_full_decomposition_error": 0.0,
            }
        ]
    )


def run_price_distance_screen(
    cfg: PriceDistanceConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_price_distance_config(cfg or configure_price_distance_run())
    ...
    print("\n===== CLV-weighted overall price distance rho=0.05 | seed 42 =====")
    ...
    stem = f"m2_clv_weighted_price_distance_{prepared['config_hash']}"
    paths = {
        ...
        "score_diagnostics_csv": prepared["out_dir"]
        / f"{stem}_score_diagnostics.csv",
    ...
    direct_table = comparison[
        (comparison.reference == ID_ONLY_MODEL_ID)
        & (comparison.model_id == MODEL_ID)
        & comparison.metric.isin(key_metrics)
    ]
    print("\n절대지표:")
    ...
    print("\n공동학습 ID-only 대비 직접 가격거리 기여:")
    print(direct_table.to_string(index=False))
    print("\nTop-10 변경 진단:")
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_price_distance_run()),
            ensure_ascii=False,
            indent=2,
        )
    )
