"""One-seed historical screen for a CLV-conditioned category-transition M3."""
...
CODE_VERSION = "m3-clv-conditioned-category-transition-historical-screen-v1"
M1_ID = "m1_baseline"
GENERAL_ID = "m3_general_first_acquisition_category_transition_control"
ACTUAL_ID = "m3_clv_conditioned_first_acquisition_category_transition"
SHUFFLE_ID = "m3_clv_conditioned_first_acquisition_category_transition_shuffle"
ARM_MODEL_IDS = {
...
class CLVCategoryTransitionConfig:
...
def configure_clv_category_transition_run(
    **overrides,
) -> CLVCategoryTransitionConfig:
    defaults = {
        "out_dir": (
    ...
    return validate_clv_category_transition_config(
        CLVCategoryTransitionConfig(**(defaults | overrides))
    )


def validate_clv_category_transition_config(
    cfg: CLVCategoryTransitionConfig,
) -> CLVCategoryTransitionConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
    ...
    for key, expected in required.items():
        ...
        if not matches:
            raise ValueError(
                f"빠른 M3 screen은 {key}={expected!r}이어야 합니다"
            )
...
def preflight_summary(cfg: CLVCategoryTransitionConfig) -> dict:
    cfg = validate_clv_category_transition_config(cfg)
    return {
        ...
        "research_question": (
            "whether train-only historical CLV conditions the direction of "
            "first-acquisition category transitions and improves new-item ranking"
        ),
        "m3": {
            "research_axis": "graph structure and propagation",
            "status": "exploratory topology extension after observed-edge reweighting failures",
            "historical_clv_proxy": "N_hat * V_hat",
            "n_hat": "number of distinct train baskets",
            "v_hat": "mean train basket value",
            "transition": (
                "source category in one basket to category of a first-acquired "
                "item in the next basket"
            ),
            "clv_relation": (
                "positive log ratio of CLV-conditioned transition probability "
                "to pooled transition probability"
            ),
            "category_level": "COMMODITY_DESC",
            "user_context": "category share in the final train basket",
            "user_transition_mass_normalized_before_clv": True,
            "cross_fit_folds": cfg.cross_fit_folds,
            "max_target_categories_per_user": cfg.max_target_categories,
            "self_history_excluded_from_consumed_transition_estimate": True,
            "kappa": cfg.kappa,
            "minimum_distinct_user_support": cfg.min_support_users,
            "log_lift_cap": cfg.log_lift_cap,
            "gamma": cfg.gamma,
            "general_transition_control": "pooled transition probability only",
            "shuffle_control": (
        ...
        "reading_rule": {
            "primary": (
                ...
                "transition, and degree-matched CLV shuffle"
            ),
...
def _base_config(cfg: CLVCategoryTransitionConfig) -> dict:
...
def _prepare(cfg: CLVCategoryTransitionConfig) -> dict:
    ...
    graph = build_clv_conditioned_category_transition_graph(
        ...
        data["n_cat"],
        kappa=cfg.kappa,
        min_support_users=cfg.min_support_users,
        log_lift_cap=cfg.log_lift_cap,
...
def _build_model(
    ...
    return CLVCategoryTransitionLightGCN(
        ...
        user_target_category=(
            prepared["graph"].user_category_operators[arm].to(v3.DEVICE)
        ),
        category_item=prepared["graph"].category_item_operator.to(v3.DEVICE),
        gamma=cfg.gamma,
        dim=cfg.id_dim,
        n_layers=cfg.n_layers,
        pref_reg=cfg.pref_reg,
...
def attribution_reading(frame: pd.DataFrame) -> dict:
    ...
    references = {
        "m1": _row(frame, M1_ID),
        "general_transition": _row(frame, GENERAL_ID),
    ...
    return {
        ...
        "six_metric_balance_actual_vs_general_transition": balances[
            "general_transition"
        ],
...
def run_clv_category_transition_screen(
    cfg: CLVCategoryTransitionConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_clv_category_transition_config(
        cfg or configure_clv_category_transition_run()
    )
    ...
    stem = f"m3_clv_category_transition_{prepared['config_hash']}"
    paths = {
