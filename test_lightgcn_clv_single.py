...
def test_hm_w60_colab_has_pinned_preset_preflight_gate_and_result_views():
    ...
    code_cells = [source for _, source in indexed_code_cells]
    source = "\n".join(code_cells)
    assert "TO_BE_PINNED" not in source

    clone_idx, clone_cell = next(
        (index, cell)
    ...
    config_idx, config_cell = next(
        (index, cell)
        for index, cell in indexed_code_cells
        if "configure_hm_60day_run" in cell
    )
    ...
    preflight_idx, preflight_cell = next(
        (index, cell)
        for index, cell in indexed_code_cells
        if "preflight_summary(cfg)" in cell
    )
    assert "summary['eval_test'] is False" in preflight_cell
    assert "summary['eval_holdout'] is False" in preflight_cell

    gate_idx, gate_cell = next(
        (index, cell)
        for index, cell in indexed_code_cells
        if "ACKNOWLEDGE_HIGH_COST =" in cell
    )
    assert "ACKNOWLEDGE_HIGH_COST = False" in gate_cell

    run_idx, run_cell = next(
        (index, cell)
        for index, cell in indexed_code_cells
        if "run_experiment(cfg)" in cell
    )
    ...
    result_idx, results_cell = next(
        (index, cell)
        for index, cell in indexed_code_cells
        if "screening_decision" in cell
    )
    assert clone_idx < config_idx < preflight_idx < gate_idx < run_idx < result_idx
