"""Seed-42 historical screen with complementary N/V training perturbations.
...
identity check.  During BPR training, the activity and transaction-value
weights move in opposite directions while their sum stays fixed.  Evaluation
uses the unperturbed 0.10 + 0.10 point.  This is exploratory development, not
another test look.
"""
...
CODE_VERSION = "m2-gatefree-lowdim-balanced-training-historical-screen-v1"
MODEL_ID = "m2_gatefree_lowdim_balanced_training"
...
class BalancedTrainingConfig:
    ...
    training_axis_balance_delta: float = 0.3
    n_layers: int = 2
    batch_size: int = 8192
    lr: float = 5e-4
    pref_reg: float = 1e-3
...
def configure_balanced_training_run(**overrides) -> BalancedTrainingConfig:
    defaults = {
        "out_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
            "_m2_gatefree_lowdim_balanced_training_historical_screen_v1"
        ),
        "baseline_result_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
    ...
    return validate_config(BalancedTrainingConfig(**(defaults | overrides)))


def validate_config(cfg: BalancedTrainingConfig) -> BalancedTrainingConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
        "time_cutoff": 690,
    ...
    if cfg.axis_budget != 0.1:
        raise ValueError("이번 screen의 평가 축별 고정 계수는 0.1이어야 합니다")
    if cfg.training_axis_balance_delta != 0.3:
        raise ValueError("이번 screen의 학습 상보 변동폭은 0.3이어야 합니다")
...
def preflight_summary(cfg: BalancedTrainingConfig) -> dict:
    ...
    return {
        ...
        "m2": {
            ...
            "training_axis_balance_delta": cfg.training_axis_balance_delta,
            "training_epsilon_distribution": "Uniform(-0.3, 0.3)",
            "training_activity_coefficient_range": [0.07, 0.13],
            "training_transaction_value_coefficient_range": [0.07, 0.13],
            "training_total_axis_budget": 0.2,
            "same_epsilon_for_positive_and_negative_in_each_triplet": True,
            "training_score_formula": (
                "S_ID + 0.1*(1+epsilon)*S_N + "
                "0.1*(1-epsilon)*S_V"
            ),
        ...
        "fixed": {
            ...
            "training_only_axis_balance_perturbation": True,
...
def _base_config(cfg: BalancedTrainingConfig) -> dict:
...
def _load_compatible_baseline(
    cfg: BalancedTrainingConfig, current_manifest: list[dict]
) -> dict:
...
def _config_hash(cfg: BalancedTrainingConfig, input_hash: str, revision: str) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
        "model_id": MODEL_ID,
...
def _prepare(cfg: BalancedTrainingConfig) -> dict:
...
def _build_model(prepared: dict, cfg: BalancedTrainingConfig):
...
def _arm_hash(prepared: dict, cfg: BalancedTrainingConfig) -> str:
...
def _run_model(prepared: dict, cfg: BalancedTrainingConfig) -> dict:
...
def run_balanced_training_screen(
    cfg: BalancedTrainingConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_config(cfg or configure_balanced_training_run())
    ...
    stem = f"m2_gatefree_lowdim_balanced_training_{prepared['config_hash']}"
    paths = {
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_balanced_training_run()),
            ensure_ascii=False,
            indent=2,
        )
    )
