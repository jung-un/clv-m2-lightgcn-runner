"""Seed-42 GAT portability screen for the established M2 embedding input."""
...
GAT_SHUFFLE = "gat_m2_degree_matched_clv_shuffle"
ACCURACY_METRICS = (
    "recall@10",
    "ndcg@10",
    "recall@20",
...
def configure_gat_clv_screen(**overrides) -> GATCLVScreenConfig:
    defaults = {
        "out_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
            "_m2_gat_clv_level_composition_price_historical_screen_v1"
        ),
        "baseline_result_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
    ...
    return validate_gat_clv_config(GATCLVScreenConfig(**(defaults | overrides)))


def validate_gat_clv_config(cfg: GATCLVScreenConfig) -> GATCLVScreenConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
        "time_cutoff": 690,
    ...
    for key, expected in required.items():
        if getattr(cfg, key) != expected:
            raise ValueError(f"GAT 개발 screen은 {key}={expected!r}이어야 합니다")
...
def preflight_summary(cfg: GATCLVScreenConfig) -> dict:
    cfg = validate_gat_clv_config(cfg)
    return {
        "code_version": CODE_VERSION,
        "dataset": cfg.dataset,
        "seed": cfg.seed,
        "research_axis": "M2 representation intervention",
        "trained_models": [GAT_M1_64, GAT_M1_67, GAT_M2, GAT_SHUFFLE],
        ...
        "m2": {
            "backbone": "single-head sparse GAT",
            "layer0": "ID(64)|CLV level-and-composition relation(2)|price(1)",
            "gat_m1_dimensions": [64, 67],
            "m2_layer0_and_final_dim": 67,
            "propagation": (
                "ELU(sum_neighbor softmax(LeakyReLU(a_dst^T Wh_v + "
                "a_src^T Wh_w)) * Wh_w)"
            ),
        ...
        "reading_rule": {
            "accuracy_floor": "all Recall/NDCG@10/20/50 >= 99% of GAT@67",
            "economic": "weighted hit@10 must exceed GAT@67",
            "semantic_attribution": (
                "observed CLV must beat degree-matched shuffle on weighted hit@10 "
                "and on either high-CLV Recall@10 or NDCG@10"
            ),
            "backbone_note": (
                "GAT portability is judged against GAT@67; LightGCN is display-only"
            ),
...
def _config_hash(cfg: GATCLVScreenConfig, input_hash: str, revision: str) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
...
def _preparation_config(cfg: GATCLVScreenConfig):
...
def _prepare(cfg: GATCLVScreenConfig) -> dict:
...
def _arm_spec(model_id: str) -> tuple[str, int, str]:
    if model_id == GAT_M1_64:
        return "id", 64, "none"
    if model_id == GAT_M1_67:
        return "id", 67, "none"
    if model_id == GAT_M2:
        return "clv", 64, "observed"
    if model_id == GAT_SHUFFLE:
        return "clv", 64, "degree_matched_shuffle"
    raise KeyError(model_id)


def _build_model(prepared: dict, cfg: GATCLVScreenConfig, model_id: str):
    ...
    model = GATCLVLevelCompositionPrice(
...
def _arm_hash(prepared: dict, cfg: GATCLVScreenConfig, model_id: str) -> str:
...
def _run_arm(
    prepared: dict, cfg: GATCLVScreenConfig, model_id: str
) -> tuple[dict, GATCLVLevelCompositionPrice]:
...
def screening_reading(metric_rows: dict[str, dict]) -> dict:
    baseline = metric_rows[GAT_M1_67]
    actual = metric_rows[GAT_M2]
    shuffled = metric_rows[GAT_SHUFFLE]
    ratios = {
    ...
    high_attribution = bool(any(actual[key] > shuffled[key] for key in high))
    return {
        ...
        "accuracy_floor_vs_gat_m1_67": accuracy_pass,
        "accuracy_ratios_vs_gat_m1_67": ratios,
        "economic_gain_vs_gat_m1_67": economic_pass,
        "weighted_hit_at_10_delta_vs_gat_m1_67": float(
...
def run_gat_clv_screen(cfg: GATCLVScreenConfig | None = None) -> pd.DataFrame:
    cfg = validate_gat_clv_config(cfg or configure_gat_clv_screen())
    print(json.dumps(preflight_summary(cfg), ensure_ascii=False, indent=2))
    prepared = _prepare(cfg)
    results: dict[str, dict] = {}
    models: dict[str, GATCLVLevelCompositionPrice] = {}
    arms = (GAT_M1_64, GAT_M1_67, GAT_M2, GAT_SHUFFLE)
    ...
    comparison = helpers._metric_comparison(
        metric_rows, references=(GAT_M1_64, GAT_M1_67, GAT_SHUFFLE)
    )

    base_users, base_top50 = helpers._masked_topk(
        models[GAT_M1_67], prepared, max_k=cfg.diagnostic_max_k
    )
    actual_users, actual_top50 = helpers._masked_topk(
        models[GAT_M2], prepared, max_k=cfg.diagnostic_max_k
    )
    shuffled_users, shuffled_top50 = helpers._masked_topk(
        models[GAT_SHUFFLE], prepared, max_k=cfg.diagnostic_max_k
    )
    if not (
        np.array_equal(base_users, actual_users)
        and np.array_equal(base_users, shuffled_users)
    ):
        raise RuntimeError("GAT arm의 평가 사용자 순서가 다릅니다")
    capacity_overlap = helpers.topk_overlap_summary(
        base_top50, actual_top50, prepared["cache"].seg, k=10
    )
    capacity_overlap.insert(0, "reference", GAT_M1_67)
    capacity_overlap.insert(1, "model_id", GAT_M2)
    shuffle_overlap = helpers.topk_overlap_summary(
        shuffled_top50, actual_top50, prepared["cache"].seg, k=10
    )
    shuffle_overlap.insert(0, "reference", GAT_SHUFFLE)
    shuffle_overlap.insert(1, "model_id", GAT_M2)
    ...
    stem = f"m2_gat_clv_level_composition_price_{prepared['config_hash']}"
    paths = {
    ...
    key_comparison = comparison[
        ...
        & comparison.reference.isin([GAT_M1_67, GAT_SHUFFLE])
        & (comparison.model_id == GAT_M2)
    ]
    print("\n1) 절대지표")
    print(absolute.to_string(index=False))
    print("\n2) GAT@67 및 degree-matched shuffle 대비 핵심 변화")
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_gat_clv_screen()),
            ensure_ascii=False,
            indent=2,
        )
    )
