...
import lightgcn_clv_neighbor_conditioned_featurewise as runner


def test_preflight_freezes_the_fast_screen_and_m2_boundaries(tmp_path):
    cfg = runner.configure_neighbor_conditioned_featurewise_run(
    ...
    assert summary["trained_models"] == [
        "m2_neighbor_conditioned_featurewise"
    ]
...
def test_historical_config_keeps_the_new_item_task(tmp_path):
    cfg = runner.configure_neighbor_conditioned_featurewise_run(
        out_dir=str(tmp_path / "new"),
        baseline_result_dir=str(tmp_path / "old"),
    )
...
def test_unplanned_variants_are_rejected(tmp_path, override, message):
    with pytest.raises(ValueError, match=message):
        runner.configure_neighbor_conditioned_featurewise_run(
            out_dir=str(tmp_path / "new"),
            baseline_result_dir=str(tmp_path / "old"),
            **override,
        )
