"""Seed-42 historical screen for a conditional-ID-transform M2.

Only the new M2 arm is trained.  A compatible M1@64 historical result is
reused after strict protocol and input-manifest checks.  This is exploratory
development and never opens the final test split.
"""
...
from clv_conditional_id_transform_model import CLVConditionalIDTransformLightGCN
...
CODE_VERSION = "m2-conditional-id-transform-historical-screen-v1"
MODEL_ID = "m2_conditional_id_transform"


@dataclass(frozen=True)
class ConditionalIDTransformConfig:
    ...
    @property
    def id_dim(self) -> int:
        """Compatibility name used by the shared historical-split helper."""
        return self.embedding_dim


def configure_conditional_id_transform_run(
    **overrides,
) -> ConditionalIDTransformConfig:
    defaults = {
        "out_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
            "_m2_conditional_id_transform_historical_screen_v1"
        ),
        "baseline_result_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
    ...
    return validate_config(ConditionalIDTransformConfig(**(defaults | overrides)))


def validate_config(cfg: ConditionalIDTransformConfig) -> ConditionalIDTransformConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
        "time_cutoff": 690,
...
def preflight_summary(cfg: ConditionalIDTransformConfig) -> dict:
    ...
    return {
        ...
        "m2": {
            "architecture": (
                "64-d user ID embedding + fixed N/V-conditioned rank-4 transforms"
            ),
            "formula": (
                "Norm(E_u + rho[c_N(u)A_N E_u + c_V(u)A_V E_u])"
            ),
            "embedding_dim": cfg.embedding_dim,
            "transform_rank": cfg.transform_rank,
            "rho": cfg.rho,
            "activity_condition": "2*train_percentile(repeat_transaction_rate)-1",
            "value_condition": "2*train_percentile(mean_transaction_value)-1",
...
def _config_hash(
    cfg: ConditionalIDTransformConfig, input_hash: str, revision: str
) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
...
def _base_config(cfg: ConditionalIDTransformConfig) -> dict:
    # This helper enforces the same historical split and all M2 boundaries as
    # the preceding low-dimensional screen.
    return previous_screen._base_config(cfg)


def _prepare(cfg: ConditionalIDTransformConfig) -> dict:
...
def _build_model(prepared: dict, cfg: ConditionalIDTransformConfig):
    data, axes = prepared["data"], prepared["axes"]
    v3.set_seed(cfg.seed)
    model = CLVConditionalIDTransformLightGCN(
...
def _arm_hash(prepared: dict, cfg: ConditionalIDTransformConfig) -> str:
...
def _run_model(prepared: dict, cfg: ConditionalIDTransformConfig) -> dict:
    ...
    if result_path.exists():
        print("  [cached] conditional-ID M2 seed 42 완료 결과 재사용")
...
def _comparison(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    metadata = {
        ...
        "activity_condition_negative_share",
        "activity_condition_positive_share",
        "value_condition_mean",
        "value_condition_std",
        "value_condition_negative_share",
        "value_condition_positive_share",
    }
    rows = []
...
def run_conditional_id_transform_screen(
    cfg: ConditionalIDTransformConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_config(cfg or configure_conditional_id_transform_run())
    print(json.dumps(preflight_summary(cfg), ensure_ascii=False, indent=2))
    prepared = _prepare(cfg)
    print("\n===== conditional-ID M2만 학습 | seed 42 | fixed 100 epochs =====")
    ...
    stem = f"m2_conditional_id_transform_{prepared['config_hash']}"
    paths = {
    ...
    reading = {
        ...
        "next_if_positive": (
            "run parameter-matched M1 and shuffled N/V controls, then 10 seeds"
        ),
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_conditional_id_transform_run()),
            ensure_ascii=False,
            indent=2,
        )
    )
