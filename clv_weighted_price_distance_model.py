"""LightGCN with one CLV-weighted overall-price-distance block."""
...
class CLVWeightedPriceDistanceLightGCN(nn.Module):
    """Jointly propagate ID coordinates and an exact price-distance basis."""
    ...
    def __init__(
        ...
        *,
        ...
        if auxiliary_dim != 2:
            raise ValueError("전체 가격거리 표현은 정확히 2차원이어야 합니다")
        if not 0.0 <= rho <= 1.0:
            raise ValueError("rho는 0 이상 1 이하여야 합니다")
        if not 0.0 < price_scale_initial < 1.0:
            raise ValueError("가격 scale 초기값은 (0,1)이어야 합니다")
        ...
        numeric = (q_c, user_overall_price, item_overall_price)
        if not all(np.isfinite(values).all() for values in numeric):
            raise ValueError("CLV·가격 입력은 모두 유한해야 합니다")
        if any(((values < 0.0) | (values > 1.0)).any() for values in numeric):
            raise ValueError("CLV·가격 입력은 [0,1] 범위여야 합니다")
        if np.any(q_c[~user_clv_valid] != 0.0):
            raise ValueError("CLV 계산 불가 사용자의 q_c는 0이어야 합니다")
        if np.any(user_overall_price[~user_price_valid] != 0.5):
            raise ValueError("가격 계산 불가 사용자의 가격 입력은 0.5여야 합니다")
        if np.any(item_overall_price[~item_price_valid] != 0.5):
            raise ValueError("가격 계산 불가 상품의 가격 입력은 0.5여야 합니다")
        ...
        # ID parameters come first so rho=0 has the same M1 initialisation.
        ...
        def buffer(name: str, values: np.ndarray) -> None:
            self.register_buffer(
                name, torch.from_numpy(values.copy()), persistent=False
            )
    ...
    def price_scale(self) -> torch.Tensor:
        """Positive learned strength with a fixed maximum of one."""

        return torch.sigmoid(self.price_scale_raw)

    def auxiliary_user_embeddings(self) -> torch.Tensor:
        valid = self.user_clv_valid * self.user_price_valid
        basis = torch.stack(
            [2.0 * self.user_overall_price, torch.ones_like(self.q_c)], dim=1
        )
        return valid[:, None] * self.q_c[:, None] * basis

    def auxiliary_item_embeddings(self) -> torch.Tensor:
        price = self.item_overall_price
        basis = torch.stack([price, -price.square()], dim=1)
        return self.item_price_valid[:, None] * self.price_scale() * basis

    def layer0_embeddings(self) -> tuple[torch.Tensor, torch.Tensor]:
        scale = math.sqrt(self.rho)
        return (
            torch.cat(
                [self.E_u.weight, scale * self.auxiliary_user_embeddings()], dim=1
            ),
            torch.cat(
                [self.E_i.weight, scale * self.auxiliary_item_embeddings()], dim=1
            ),
        )
    ...
    def layer0_auxiliary_scores(
        self, users: torch.Tensor, items: torch.Tensor
    ) -> torch.Tensor:
        user = self.auxiliary_user_embeddings()[users]
        item = self.auxiliary_item_embeddings()[items]
        return self.rho * (user * item).sum(1)
    ...
    def epoch_training_diagnostics(self) -> dict[str, float]:
        ...
        keys = (
            ...
            "layer0_pairwise_intervention_bound",
        )
    ...
    def representation_diagnostics(self) -> dict[str, float | int | bool]:
        ...
        return {
            ...
            "historical_clv_definition": "q_C=percentile(N_hat*V_hat)",
            "overall_price_distance_only": True,
            "exact_layer0_bpr_price_distance_basis": True,
            ...
            "price_scale_max": 1.0,
            "layer0_pairwise_intervention_bound": self.rho * price_scale,
