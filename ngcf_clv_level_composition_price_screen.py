"""Seed-42 NGCF screen for the fixed CLV level/composition/price embedding."""
...
CODE_VERSION = "m2-ngcf-clv-level-composition-price-historical-screen-v1"
NGCF_M1_64 = "ngcf_m1_64"
NGCF_M1_67 = "ngcf_m1_67"
NGCF_M2 = "ngcf_m2_clv_level_composition_price"
NGCF_SHUFFLE = "ngcf_m2_degree_matched_clv_shuffle"
ACCURACY_METRICS = (
    "recall@10",
    "ndcg@10",
    "recall@20",
...
def configure_ngcf_clv_screen(**overrides) -> NGCFCLVScreenConfig:
    defaults = {
        "out_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
            "_m2_ngcf_clv_level_composition_price_historical_screen_v1"
        ),
        "baseline_result_dir": (
            f"{v3.default_out_dir('dunnhumby')}"
    ...
    return validate_ngcf_clv_config(
        NGCFCLVScreenConfig(**(defaults | overrides))
    )


def validate_ngcf_clv_config(cfg: NGCFCLVScreenConfig) -> NGCFCLVScreenConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
        "time_cutoff": 690,
    ...
    for key, expected in required.items():
        if getattr(cfg, key) != expected:
            raise ValueError(f"NGCF 개발 screen은 {key}={expected!r}이어야 합니다")
...
def preflight_summary(cfg: NGCFCLVScreenConfig) -> dict:
    cfg = validate_ngcf_clv_config(cfg)
    return {
        "code_version": CODE_VERSION,
        "dataset": cfg.dataset,
        "seed": cfg.seed,
        "research_axis": "M2 representation intervention",
        "trained_models": [NGCF_M1_64, NGCF_M1_67, NGCF_M2, NGCF_SHUFFLE],
        ...
        "m2": {
            ...
            "ngcf_m1_dimensions": [64, 67],
            "m2_layer0_dim": 67,
            "m2_final_dim": 67 * (cfg.n_layers + 1),
            "propagation": (
                "LeakyReLU(W_sum(E+A_hat E)+W_bi(E elementwise A_hat E))"
            ),
            "layer_outputs": "layer0, layer1, layer2 concatenation",
            "rho": cfg.rho,
        ...
        "reading_rule": {
            "accuracy_guardrail": "all six actual/M1@67 ratios >= 0.99",
            "top10": "actual Recall@10 and NDCG@10 > M1@67",
            "high_clv": "actual high-CLV Recall@10 and NDCG@10 > M1@67",
            "economic": "actual weighted hit@10 > M1@67",
            "attribution": (
                "actual > degree-matched shuffle on six-metric geometric mean, "
                "high-CLV Recall/NDCG@10, and weighted hit@10"
            ),
            "if_failed": "stop without tuning NGCF or CLV settings on days 684-690",
...
def _config_hash(cfg: NGCFCLVScreenConfig, input_hash: str, revision: str) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
...
def _preparation_config(cfg: NGCFCLVScreenConfig):
...
def _prepare(cfg: NGCFCLVScreenConfig) -> dict:
...
def _arm_spec(model_id: str) -> tuple[str, int, str]:
    if model_id == NGCF_M1_64:
        return "id", 64, "none"
    if model_id == NGCF_M1_67:
        return "id", 67, "none"
    if model_id == NGCF_M2:
        return "clv", 64, "observed"
    if model_id == NGCF_SHUFFLE:
        return "clv", 64, "degree_matched_shuffle"
    raise KeyError(model_id)


def _build_model(prepared: dict, cfg: NGCFCLVScreenConfig, model_id: str):
    ...
    model = NGCFCLVLevelCompositionPrice(
...
def _arm_hash(prepared: dict, cfg: NGCFCLVScreenConfig, model_id: str) -> str:
...
def _run_arm(
    prepared: dict, cfg: NGCFCLVScreenConfig, model_id: str
) -> tuple[dict, NGCFCLVLevelCompositionPrice]:
...
def screening_reading(metric_rows: dict[str, dict]) -> dict:
    baseline = metric_rows[NGCF_M1_67]
    actual = metric_rows[NGCF_M2]
    shuffled = metric_rows[NGCF_SHUFFLE]
    ratios_vs_capacity = {
        metric: _safe_ratio(actual[metric], baseline[metric])
        for metric in ACCURACY_METRICS
    }
    ratios_vs_shuffle = {
        metric: _safe_ratio(actual[metric], shuffled[metric])
        for metric in ACCURACY_METRICS
    }
    finite_shuffle_ratios = [
        value for value in ratios_vs_shuffle.values() if np.isfinite(value) and value > 0
    ]
    accuracy_geomean_vs_shuffle = (
        float(np.exp(np.mean(np.log(finite_shuffle_ratios))))
        if len(finite_shuffle_ratios) == len(ACCURACY_METRICS)
        else float("nan")
    )
    high_metrics = ("고CLV_recall@10", "고CLV_ndcg@10")
    weighted_metric = "price_purchase_amount_weighted_hit@10"
    positive = bool(
        min(ratios_vs_capacity.values()) >= 0.99
        and actual["recall@10"] > baseline["recall@10"]
        and actual["ndcg@10"] > baseline["ndcg@10"]
        and all(actual[key] > baseline[key] for key in high_metrics)
        and actual[weighted_metric] > baseline[weighted_metric]
        and accuracy_geomean_vs_shuffle > 1.0
        and all(actual[key] > shuffled[key] for key in high_metrics)
        and actual[weighted_metric] > shuffled[weighted_metric]
    )
    return {
        "positive_screen": positive,
        "accuracy_ratios_vs_ngcf_m1_67": ratios_vs_capacity,
        "accuracy_ratios_vs_degree_matched_shuffle": ratios_vs_shuffle,
        "accuracy_geomean_ratio_vs_degree_matched_shuffle": (
            accuracy_geomean_vs_shuffle
        ),
        "top10_better_than_ngcf_m1_67": bool(
            actual["recall@10"] > baseline["recall@10"]
            and actual["ndcg@10"] > baseline["ndcg@10"]
        ),
        "high_clv_better_than_ngcf_m1_67": bool(
            all(actual[key] > baseline[key] for key in high_metrics)
        ),
        "high_clv_better_than_degree_matched_shuffle": bool(
            all(actual[key] > shuffled[key] for key in high_metrics)
        ),
        "weighted_hit_at_10_delta_vs_ngcf_m1_67": float(
            actual[weighted_metric] - baseline[weighted_metric]
        ),
        "weighted_hit_at_10_delta_vs_degree_matched_shuffle": float(
            actual[weighted_metric] - shuffled[weighted_metric]
        ),
        "next_if_positive": "paired development seeds, then H&M",
        "if_failed": "stop without tuning on days 684-690",
        "statistical_note": "seed 42 exploratory screen; no significance claim",
    }


def run_ngcf_clv_screen(
    cfg: NGCFCLVScreenConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_ngcf_clv_config(cfg or configure_ngcf_clv_screen())
    print(json.dumps(preflight_summary(cfg), ensure_ascii=False, indent=2))
    prepared = _prepare(cfg)
    results: dict[str, dict] = {}
    models: dict[str, NGCFCLVLevelCompositionPrice] = {}
    for model_id in (NGCF_M1_64, NGCF_M1_67, NGCF_M2, NGCF_SHUFFLE):
        print(f"\n===== {model_id} | seed 42 | fixed 100 epochs =====")
        results[model_id], models[model_id] = _run_arm(prepared, cfg, model_id)

    metric_rows = {model_id: result["metrics"] for model_id, result in results.items()}
    ...
    comparison = helpers._metric_comparison(
        metric_rows,
        references=(NGCF_M1_64, NGCF_M1_67, NGCF_SHUFFLE),
    )

    base_users, base_top50 = helpers._masked_topk(
        models[NGCF_M1_67], prepared, max_k=cfg.diagnostic_max_k
    )
    actual_users, actual_top50 = helpers._masked_topk(
        models[NGCF_M2], prepared, max_k=cfg.diagnostic_max_k
    )
    shuffled_users, shuffled_top50 = helpers._masked_topk(
        models[NGCF_SHUFFLE], prepared, max_k=cfg.diagnostic_max_k
    )
    if not (
        np.array_equal(base_users, actual_users)
        and np.array_equal(base_users, shuffled_users)
    ):
        raise RuntimeError("NGCF arm의 평가 사용자 순서가 다릅니다")
    capacity_overlap = helpers.topk_overlap_summary(
        base_top50, actual_top50, prepared["cache"].seg, k=10
    )
    capacity_overlap.insert(0, "reference", NGCF_M1_67)
    capacity_overlap.insert(1, "model_id", NGCF_M2)
    shuffle_overlap = helpers.topk_overlap_summary(
        shuffled_top50, actual_top50, prepared["cache"].seg, k=10
    )
    shuffle_overlap.insert(0, "reference", NGCF_SHUFFLE)
    shuffle_overlap.insert(1, "model_id", NGCF_M2)
    ...
    stem = f"m2_ngcf_clv_level_composition_price_{prepared['config_hash']}"
    paths = {
    ...
    key_metrics = (*ACCURACY_METRICS, "price_purchase_amount_weighted_hit@10", "고CLV_recall@10", "고CLV_ndcg@10")
    key_comparison = comparison[
        comparison.metric.isin(key_metrics)
        & comparison.reference.isin([NGCF_M1_67, NGCF_SHUFFLE])
        & (comparison.model_id == NGCF_M2)
    ]
    print("\n1) 절대지표")
    print(absolute.to_string(index=False))
    print("\n2) NGCF@67 및 degree-matched shuffle 대비 핵심 변화")
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_ngcf_clv_screen()),
            ensure_ascii=False,
            indent=2,
        )
    )
