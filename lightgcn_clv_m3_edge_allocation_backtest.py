"""One-seed historical backtest for CLV allocation over graph edges.

This exploratory run never constructs the already-inspected final test.  It
trains on Dunnhumby DAY 1--683 and evaluates the fixed DAY 684--690 historical
development interval once after exactly 100 epochs.
"""
...
CODE_VERSION = "m3-clv-edge-allocation-historical-pilot-v1"
MODEL_IDS = ("m1_baseline", "m3_clv_edge_allocation")
ACCURACY_METRICS = (
    "recall@10",
    "ndcg@10",
    "recall@20",
...
def _default_out_dir() -> str:
    ...
    return f"{v3.default_out_dir('dunnhumby')}_m3_clv_edge_allocation_historical"


def configure_m3_edge_allocation_backtest(
    **overrides,
) -> M3EdgeAllocationBacktestConfig:
...
def validate_config(
    cfg: M3EdgeAllocationBacktestConfig,
) -> M3EdgeAllocationBacktestConfig:
    fixed = {
        "dataset": "dunnhumby",
        "seed": 42,
    ...
    for key, expected in fixed.items():
        if getattr(cfg, key) != expected:
            raise ValueError(f"historical M3 pilot requires {key}={expected!r}")
...
def preflight_summary(cfg: M3EdgeAllocationBacktestConfig) -> dict:
    ...
    return {
        ...
        "new_item_task": (
            "all DAY 1--683 train pairs are excluded from evaluation truth and Top-K"
        ),
        "graph_intervention": {
            "support": "same sorted unique train user-item edges as M1",
            "historical_clv_proxy": "N_hat * V_hat",
            "relationship": "price-free TF-IDF-shaped basket frequency",
            "user_from_item": "unchanged M1 coefficient",
            "item_from_user": "edge-specific CLV allocation with fixed item mass",
        ...
        "interpretation": (
            "exploratory one-seed historical backtest; no variance, interval, "
...
def _base_config(cfg: M3EdgeAllocationBacktestConfig) -> dict:
    ...
    for key, expected in required.items():
        if base.get(key) != expected:
            raise RuntimeError(f"historical M3 configuration contamination: {key}")
    return base


def _config_hash(
    cfg: M3EdgeAllocationBacktestConfig, input_hash: str, revision: str
) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
...
def _prepare(cfg: M3EdgeAllocationBacktestConfig) -> dict:
    ...
    if float(data["train"]["t"].max()) != 683.0:
        raise RuntimeError("historical train must end at DAY 683")
    if data.get("loss_w") is not None:
        raise RuntimeError("M4 loss weighting contaminated the M3 pilot")
    data["loss_w"] = None
    stats = data.get("data_stats", {})
    split_status = stats.get("split_evaluation_status", {})
    if split_status.get("holdout") != "not_constructed":
        raise RuntimeError("holdout must not be constructed")
    if float(stats.get("source", {}).get("time_max", -1)) != 690.0:
        raise RuntimeError("rows after DAY 690 must not enter the pilot")

    graph = build_edge_allocated_clv_graph(
    ...
    if not (
        ...
        raise RuntimeError("M3 unique edge support or order differs from M1")

    stats["m3_clv_edge_allocation"] = graph.diagnostics
...
def _build_model(
    prepared: dict, cfg: M3EdgeAllocationBacktestConfig, model_id: str
):
    data = prepared["data"]
    if model_id not in MODEL_IDS:
        raise KeyError(model_id)
    model_data = data
    if model_id == "m3_clv_edge_allocation":
        model_data = {
            **data,
...
def _arm_paths(prepared: dict, cfg: M3EdgeAllocationBacktestConfig, model_id: str):
...
def _pilot_decision(frame: pd.DataFrame) -> dict:
    ...
    accuracy = {
        metric: float(model[metric] / base[metric])
...
def run_m3_edge_allocation_backtest(
    cfg: M3EdgeAllocationBacktestConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_config(cfg or configure_m3_edge_allocation_backtest())
    ...
    stem = f"m3_clv_edge_allocation_backtest_{prepared['config_hash']}"
    paths = {
    ...
    test10._atomic_json(
        paths["json"],
        {
            ...
            "arms": [{key: value for key, value in arm.items() if key != "training"}
                     | {"training": arm["training"]} for arm in arms],
            "absolute_rows": frame.to_dict("records"),
            "comparison_rows": comparison.to_dict("records"),
            "pilot_decision": decision,
            "result_paths": {key: str(value) for key, value in paths.items()},
            "interpretation": (
                "exploratory one-seed historical backtest; passing only opens "
                "the controls, ten-seed, or external-dataset stage"
            ),
        },
    )
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_m3_edge_allocation_backtest()),
            ensure_ascii=False,
            indent=2,
        )
    )
