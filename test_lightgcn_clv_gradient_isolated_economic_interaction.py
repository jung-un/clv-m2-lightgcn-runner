...
from lightgcn_clv_gradient_isolated_economic_interaction import (
    MATCHED_MODEL_ID,
    MODEL_ID,
    build_clv_inputs,
    build_item_price_inputs,
    configure_gradient_isolated_run,
    preflight_summary,
    screening_reading,
...
def test_preflight_freezes_m2_boundaries_and_matched_control():
    cfg = configure_gradient_isolated_run(
...
def test_clv_inputs_keep_n_v_and_overall_level_separate():
    axes = {
    ...
    np.testing.assert_allclose(q_n[:3], [0.2, 0.8, 0.9])
    np.testing.assert_allclose(q_v[:3], [0.9, 0.8, 0.1])
    assert q_c[3] == 0.0 and q_n[3] == 0.0 and q_v[3] == 0.0
    assert not valid[3]


def test_item_price_inputs_are_train_only_percentiles():
    train = pd.DataFrame(
        {
            "i_idx": [0, 0, 1, 2],
            "up": [10.0, 20.0, 30.0, 50.0],
        }
    )
    price, valid = build_item_price_inputs(train, n_items=4)

    assert valid.tolist() == [True, True, True, False]
    assert price[0] < price[1] < price[2]
    assert price[3] == 0.5
...
def test_screen_requires_accuracy_high_clv_economic_and_mechanism_conditions():
    ...
    reading = screening_reading(
        matched, model, matched, overlap, {"rho_zero_auxiliary_max_abs": 0.0}
    )
