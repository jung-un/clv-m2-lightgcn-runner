"""Seed-42 historical screen for the neighbour-conditioned M2 expression."""
...
import lightgcn_clv_conditional_id_transform as previous
...
class NeighborConditionedIDTransformConfig:
...
def configure_neighbor_conditioned_id_transform_run(
    **overrides,
) -> NeighborConditionedIDTransformConfig:
    defaults = {
        "out_dir": (
    ...
    return validate_config(
        NeighborConditionedIDTransformConfig(**(defaults | overrides))
    )


def validate_config(
    cfg: NeighborConditionedIDTransformConfig,
) -> NeighborConditionedIDTransformConfig:
    required = {
        "dataset": "dunnhumby",
        "seed": 42,
...
def preflight_summary(cfg: NeighborConditionedIDTransformConfig) -> dict:
    ...
    return {
        ...
        "m2": {
            "architecture": (
                "64-d user ID + centred N/V-conditioned rank-4 transforms "
                "of the one-hop learned-item-ID aggregate"
            ),
            "formula": (
                "Norm(E_u + rho[D_N(u)+D_V(u)]), "
                "D_k=centre_valid(q_k(u) A_k Norm(sum_i Ahat_ui E_i))"
            ),
        ...
        "fixed": {
            ...
            "loss": "one plain pairwise BPR plus the existing L2 term",
            "new_loss_term": False,
...
def _config_hash(
    cfg: NeighborConditionedIDTransformConfig, input_hash: str, revision: str
) -> str:
    payload = {
        "code_version": CODE_VERSION,
        "config": asdict(cfg),
...
def _base_config(cfg: NeighborConditionedIDTransformConfig) -> dict:
    return previous._base_config(cfg)


def _prepare(cfg: NeighborConditionedIDTransformConfig) -> dict:
    # Reuse the verified historical data construction and compatible M1 loader.
...
def _build_model(prepared: dict, cfg: NeighborConditionedIDTransformConfig):
...
def _arm_hash(prepared: dict, cfg: NeighborConditionedIDTransformConfig) -> str:
...
def _run_model(prepared: dict, cfg: NeighborConditionedIDTransformConfig) -> dict:
    ...
    if result_path.exists():
        print("  [cached] neighbour-conditioned M2 seed 42 완료 결과 재사용")
...
def run_neighbor_conditioned_id_transform_screen(
    cfg: NeighborConditionedIDTransformConfig | None = None,
) -> pd.DataFrame:
    cfg = validate_config(cfg or configure_neighbor_conditioned_id_transform_run())
    print(json.dumps(preflight_summary(cfg), ensure_ascii=False, indent=2))
    prepared = _prepare(cfg)
    print("\n===== neighbour-conditioned M2만 학습 | seed 42 | fixed 100 epochs =====")
    ...
    stem = f"m2_neighbor_conditioned_id_transform_{prepared['config_hash']}"
    paths = {
...
if __name__ == "__main__":
    print(
        json.dumps(
            preflight_summary(configure_neighbor_conditioned_id_transform_run()),
            ensure_ascii=False,
            indent=2,
        )
    )
