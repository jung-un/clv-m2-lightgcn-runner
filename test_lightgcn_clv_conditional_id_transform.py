...
import lightgcn_clv_conditional_id_transform as runner


def test_preflight_freezes_the_approved_architecture_and_boundaries(tmp_path):
    cfg = runner.configure_conditional_id_transform_run(
        out_dir=str(tmp_path / "new"),
        baseline_result_dir=str(tmp_path / "old"),
    )
    summary = runner.preflight_summary(cfg)

    assert summary["trained_models"] == ["m2_conditional_id_transform"]
    ...
    assert summary["m2"]["both_axes_always_present"] is True
    assert summary["m2"]["separate_axis_score_space"] is False
...
def test_shared_historical_config_keeps_new_item_task_and_m2_boundaries(tmp_path):
    cfg = runner.configure_conditional_id_transform_run(
        out_dir=str(tmp_path / "new"),
        baseline_result_dir=str(tmp_path / "old"),
    )
...
def test_unplanned_screen_variants_are_rejected(tmp_path, override, message):
    with pytest.raises(ValueError, match=message):
        runner.configure_conditional_id_transform_run(
            out_dir=str(tmp_path / "new"),
            baseline_result_dir=str(tmp_path / "old"),
            **override,
        )
